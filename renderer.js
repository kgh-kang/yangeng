...
function _rkClauseSingle(node, opt) {
    ...
    cells.forEach((c, i) => {                                 // 구분선
        ...
        if (prev.isClause || c.isClause) return;             // 명사절 주어/목적어는 받침대 기둥이 경계를 대신
    ...
    cells.forEach((c, ci) => {                                // 셀 단어(+미니술어 │목적어 / 명사절 받침대)
        if (c.isClause) {                                    // 명사절: 받침대 위 미니 절 + 기둥(인접 성분과의 경계 겸용)
            const clauseY = baseY - standGap - c.clauseR.h;
            const isSubj = (ci === 0);                         // 주어절=오른쪽 경계(뒤에 V), 목적어/보어절=왼쪽 경계(앞에 V)
            const boundaryX = isSubj ? (c.x + c.cellW) : c.x;
            const clauseX = isSubj ? Math.max(0, boundaryX - c.clauseR.w - 4) : (boundaryX + 4);
            svg += _g(clauseX, clauseY, c.clauseR.svg);
            svg += _line(boundaryX, baseY + sz * 0.5, boundaryX, baseY - standGap, 2.0, mc);   // 기둥=성분 경계 겸용
            // 연결어 라벨: that/whether처럼 절 성분이 아닌 진짜 접속사일 때만. wh-어가 주어면(이미 절에 표시) 생략
            const cnS = c.clauseNode.S || c.clauseNode.s;
            const cnSw = (cnS && typeof cnS === 'object') ? (cnS.w || cnS.head) : cnS;
            if (c.connector && String(c.connector).toLowerCase() !== String(cnSw || '').toLowerCase())
                svg += _txt(boundaryX + 5, baseY - standGap + 1, c.connector, Math.max(11, sz - 6), _RK.sub);
    ...
    return { svg, w: maxRight, h: maxBottom };
}

function buildTreeSVG(node) {
    const r = _rkClause(node, { size: 18, color: _RK.line });
